...
class Utils {
  ...
  static Future<LocationResult?> showPlacePickerWithLatLong(
    ...
    LocationResult? result = await Navigator.of(context).push(MaterialPageRoute(
        builder: (context) => PlacePicker(
              "AIzaSyANzLAh850Y-igWJKaJA-UWUzNwHyOeq4I",
  ...
  Future<PlacesDetailsResponse?> handlePressButton(BuildContext context) async {
    void onError(PlacesAutocompleteResponse response) {
      // Handle the error from the autocomplete request
    ...
    final p = await PlacesAutocomplete.show(
      context: context,
      apiKey: Constant.mapAPIKey.toString(),
      onError: onError,
      mode: Mode.overlay,
      language: 'en',
    ...
    try {
      ...
      // Handle errors from the getDetailsByPlaceId request
      ShowToastDialog.showToast('Error fetching place details: $error');
      ShowToastDialog.closeLoader();
      return null;
    }
  }
